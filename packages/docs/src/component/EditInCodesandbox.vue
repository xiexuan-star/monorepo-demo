<template>
	<form action="https://codesandbox.io/api/v1/sandboxes/define" method="POST" target="_blank" style="display: flex">
		<input type="hidden" name="parameters" :value="parameters" />
		<n-button class="edit-button" text :size="size" attr-type="submit" depth="3">
			<template #icon>
				<n-icon>
...
import { useCodeSandbox } from '../hooks';
...
const parameters = computed(() => {
	return useCodeSandbox().getCodeSandboxParams(props.code, props.title);
});
</script>
